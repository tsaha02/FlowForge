...
// ─── Shared input styles ─────────────────────────────────────
const inputBase = (focused: boolean, mono = false): React.CSSProperties => ({
  width: '100%',
  padding: '9px 12px',
...
const textareaBase = (focused: boolean, mono = false, dark = false): React.CSSProperties => ({
  width: '100%',
  padding: '9px 12px',
...
export default function NodeConfigPanel() {
  const { nodes, selectedNodeId, setSelectedNode, updateNodeData, deleteNode, addNode } = useWorkflowStore();
            ...
            {typeDef.fields.map(field => (
              <div key={field.name}>
                ...
                {(field.type === 'textarea' || field.type === 'json') && (
                  <FocusField>
                    {(focused, setFocused) => (
                      <textarea value={(config[field.name] as string) || ''} onChange={e => handleFieldChange(field.name, e.target.value)} placeholder={field.placeholder} rows={3} onFocus={() => setFocused(true)} onBlur={() => setFocused(false)} style={textareaBase(focused, true)} />
                    )}
                  </FocusField>
                )}
                ...
                {field.type === 'code' && (
                  <FocusField>
                    {(focused, setFocused) => (
                      <textarea value={(config[field.name] as string) || ''} onChange={e => handleFieldChange(field.name, e.target.value)} placeholder={field.placeholder} rows={5} onFocus={() => setFocused(true)} onBlur={() => setFocused(false)} style={textareaBase(focused, true, true)} />
                    )}
                  </FocusField>
                )}
